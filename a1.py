...
def get_repos(repo_count, max_commit_count):
    headers = {AUTHERIZATION: +TOKEN_VALUE}
    ...
    for url in df[COMMITS_URL]:
        ...
        nested_author = commit.apply(lambda x: x[AUTHOR])
        nested_committer = commit.apply(lambda x: x[COMMITTER])
        
        commit_df[AUTHOR_NAME] = nested_author.apply(lambda x: x[NAME])
        commit_df[AUTHOR_EMAIL] = nested_author.apply(lambda x: x[EMAIL])
        commit_df[COMMITTER_NAME] = nested_committer.apply(lambda x: x[NAME])
        commit_df[COMMITTER_EMAIL] = nested_committer.apply(lambda x: x[EMAIL])

        commit_df[AUTHOR_USERNAME] = author.apply(lambda x: x[LOGIN])
        commit_df[AUTHOR_URL] = author.apply(lambda x: x[HTML_URL])
        commit_df[COMMITTER_USERNAME] = committer.apply(lambda x: x[LOGIN])
        commit_df[COMMITTER_URL] = committer.apply(lambda x: x[HTML_URL])
        
        commit_df[COMMIT_MESSAGE] = commit.apply(lambda x: x[MESSAGE])
